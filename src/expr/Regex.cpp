...
#define STATE_CALL(st, ret) \
	stateStack.push_back(ret); \
	state = st
#define STATE_RETURN \
	...
	while (pos < expression.length()) {
		...
		case RegexState::ExplicitQuantifier2:
			...
			case '}':
				...
				top = seqStack.back();
				seqStack.pop_back();
				if (codeChar == 0) codeChar = UINT_MAX;
		...
		case RegexState::Class:
			...
			switch (c) {
				...
				case '-':
					if (lastChar != UINT_MAX) {	// when there is a char before, it's a range
						state = RegexState::ClassRange;
					} else { // else it's just the '-' character
						lastChar = '-';
					}
					break;
...
// TODO: 999 Create a Test Fixture for the Regex class.
