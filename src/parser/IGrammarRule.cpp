...
namespace pgen {
	...
	IGrammarRule::~IGrammarRule() {
		for(auto symSeq: symbols) {
			if (symSeq != nullptr) delete symSeq;
		}
	}
			...
			if (node.IsScalar()) {
				...
				if (items.size() > 0) {
					...
					for (string item: items) {
						...
						if (symbolId == -1) symbolId = language->getNonTerminalId(item);
						if (symbolId == -1) throw new LanguageException("Unrecognized symbol: " + item);
