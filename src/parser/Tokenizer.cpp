 ...
 * The only field of the language that is used by the Tokenizer is the "prefix" field, which should be something like
 * "ML_".
 */
 ...
 * @remark the ownership of the tokentype is transfered to the tokenizer. i.e. the user doesn't need to delete it. In 
 * fact, the user must not delete it, and it must not be a stackframe object which will get released upon the end of 
 * some method.
 */
 ...
 * It currently needs the token struct which consists of a int type and a value pointer. And also a token_list structure
 * which consists of token struct items, a pointer to the global data section used by the structure for all data of all
 * tokens, a count of tokens and a data_ptr which tells the current pointer at the data structure.
 * @param stringstream the stringstream to write to
 */
 ...
 * I believe this is faster than allocating dynamic structures to keep the tokens as they are parsed, because it reduces
 * the memory fragmentation by allocating a single huge block of memory to keep all the data.
 * @param stringstream the stringstream to write to
 */
...
void Tokenizer::codeTokenizeString(stringstream &s) {
	...
	s << "\treturn " << fnNameTokenizeStringLen() << "(text, (int)strlen_utf8(text));\n"
		 "}\n\n";
}

//----------------------------------------------------------------------------------------------------------------------
/**
 * Writes the tokenize_file method, which tokenizes all the text on a given file. The method currently reads the entire
 * file to a memory buffer and then uses the tokenize_string_len method to tokenize it. The user need to have enough
 * memory to keep the entire file buffer.
 * @param stringstream the stringstream to write to
 */
